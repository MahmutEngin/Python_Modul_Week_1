...
n = int(input("Kaç terimlik Fibonacci dizisi istersiniz? "))

fibonacci = [0, 1]
for i in range(2, n):
    sonraki = fibonacci[-1] + fibonacci[-2]
    fibonacci.append(sonraki)
...
kelimeters=input('Tersi çevirmek istediğiniz kelimeyi giriniz: ')
kelimeters=kelimeters[::-1]
print(kelimeters)
...
kelime_palindrom = input("Bir kelime girin: ")
kelime_ters = kelime_palindrom[::-1]
if kelime_palindrom == kelime_ters:
...
if vucut_kitle_indeksi < 25:
    ...
    print("Obez")
print("Vücut kitle indeksiniz: ", vucut_kitle_indeksi)
...
print("En büyük sayı: ", min(sayi1,sayi2,sayi3))
...
for i in range(1, 5):
    ...
    print(f"{i}. Ders Ortalaması: {ortalama}")
    
    if ortalama >= 50:
        print("Durum: SUCCESSFUL (BAŞARILI)")
    else:
        print("Durum: FAILED (BAŞARISIZ)")